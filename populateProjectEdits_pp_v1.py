...
class populateProjectEdits(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            # Get page ids for all project sub-pages
            query = "SELECT tp_id FROM ts_pages WHERE tp_title LIKE %s AND tp_namespace IN (4,5)"
            db.execute(self.cursor, query, (self.project[1] + "%", ))
            rows = self.cursor.fetchall()
            ids = []
            # If we didn't find a sub-page, just search for the project page id.
            if len(rows) == 0:
                ids += [self.project[0]]
            else:
                for row in rows:
            ...
            print self.getName() + " - Project: " + self.project[1] + " - Found " + str(len(ids)) + " sub-pages."

            # Then get all the edits to the project and sub-pages in both WP and WP_Talk namespaces
            query = "SELECT rc_user_id, rc_edits, rc_wikiweek FROM reflex_cache LEFT JOIN ts_users_groups ON tug_uid = rc_user_id WHERE rc_page_id IN (" + s_ids + ") AND rc_user_id > 0 AND (tug_group NOT LIKE 'bot%%' OR tug_group IS NULL) ORDER BY rc_wikiweek ASC"
            ...
            while True:
                ...
                for row in rows:
                    ...
                    if quarter != last_quarter and last_quarter != 0:
                        ...
                        c1.execute('INSERT INTO cscw_project_edits_pp_v1 (cpe_project_id, cpe_project_page_edits, cpe_project_page_editors, cpe_quarter) VALUES (%s,%s,%s,%s) ON DUPLICATE KEY UPDATE cpe_project_id = cpe_project_id', (self.project[0], edits, len(editors), last_quarter))
                        edits = 0
                        editors = {}

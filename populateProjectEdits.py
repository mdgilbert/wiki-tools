...
class populateProjectEdits(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            query = "SELECT rc_wikiweek FROM reflex_cache WHERE rc_page_id = %s ORDER BY rc_wikiweek ASC LIMIT 1"
            db.execute(self.cursor, query, (self.project[0], ))
            row = self.cursor.fetchone()
            create_week = row[0]

            print self.getName() + " - Project: " + self.project[1] + ", Create Week: " + str(create_week) + ", Create Quarter: " + str(convertWeekToQuarter(create_week))

            # Then get all the edits to the project pages from that time on
            # This gets us all the edits to any page owned by the project:
            ...
            query = "SELECT rc_user_id, rc_edits, rc_wikiweek FROM n_project_pages JOIN reflex_cache ON pp_id = rc_page_id LEFT JOIN ts_users_groups ON tug_uid = rc_user_id WHERE pp_project_id = %s AND rc_wikiweek >= %s AND rc_user_id > 0 AND (tug_group NOT LIKE 'bot%%' OR tug_group IS NULL) ORDER BY rc_wikiweek ASC"
            self.cursor.execute(query, (self.project[0], create_week))
            edits = 0
            editors = {}
            last_quarter = 0
            while True:
                ...
                for row in rows:
                    ...
                    if quarter != last_quarter and last_quarter != 0:
                        ...
                        print self.getName() + " - Inserting rows for project id " + str(self.project[0]) + " for quarter " + str(last_quarter)
                        c1 = db.getCursorForDB("reflex_relations", self.getName() + "_insert")
                        c1.execute('INSERT INTO cscw_project_edits (cpe_project_id, cpe_project_page_edits, cpe_project_page_editors, cpe_quarter) VALUES (%s,%s,%s,%s) ON DUPLICATE KEY UPDATE cpe_project_id = cpe_project_id', (self.project[0], edits, len(editors), last_quarter))
                        edits = 0
                        editors = {}
